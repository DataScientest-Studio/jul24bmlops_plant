...
async def backup_db(db: Session = Depends(get_db), token: str = Depends(get_token_from_request)):
    try:
...
async def restore_db(db: Session = Depends(get_db), token: str = Depends(get_token_from_request)):
    try:
        current_admin_user = await get_current_admin_user(token)
        # restore_database()
